...
4. **成功時**: アプリ独自によるSDカードへのデータ書き込みは行わず終了（※動作確認用に /tmp/pipulse/latest_pipulse.txt などの揮発キャッシュを残すのみ）。
...
- 通信失敗分は次回起動時に flush され、`received_at` は再送成功時刻になる。
- `timestamp` は測定時刻のまま残る（例: 測定 11:30:03 → 受信 11:45:06）。
一覧を received_at や、旧 show_latest.sh のように COALESCE(received_at, timestamp) を優先で見ると、欠測＋同秒二重に見えても、実体は遅延到着のことがある。
- 欠測判定や時系列の確認は **`timestamp`（測定時刻）を優先**する。
- 電源断・再起動時のキュー揮発は §10.6 のとおり許容する（意図的トレードオフ）。
